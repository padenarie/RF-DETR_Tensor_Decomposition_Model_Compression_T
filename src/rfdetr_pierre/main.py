...
import rfdetr_pierre.util.misc as utils
from rfdetr_pierre.assets.model_weights import (
    ModelWeights,
    download_pretrain_weights,
    validate_pretrain_weights,
)
from rfdetr_pierre.datasets import build_dataset, get_coco_api_from_dataset
from rfdetr_pierre.engine import evaluate, train_one_epoch
from rfdetr_pierre.models import PostProcess, build_criterion_and_postprocessors, build_model
from rfdetr_pierre.util.benchmark import benchmark
from rfdetr_pierre.util.drop_scheduler import drop_scheduler
from rfdetr_pierre.util.get_param_dicts import get_param_dict
from rfdetr_pierre.util.logger import get_logger
from rfdetr_pierre.util.misc import get_rank, get_world_size, is_main_process, save_on_master
from rfdetr_pierre.util.utils import BestMetricHolder, ModelEma, clean_state_dict
from rfdetr_pierre.utilities.decorators import _DeprecatedDict
...
class Model:
    def __init__(self, **kwargs):
        ...
        if args.pretrain_weights is not None:
            ...
            if args.pretrain_keys_modify_to_load is not None:
                from rfdetr_pierre.util.obj365_to_coco_model import get_coco_pretrain_from_obj365
    ...
    def train(self, callbacks: DefaultDict[str, List[Callable]], **kwargs):
        ...
        if args.output_dir and is_main_process() and getattr(args, "save_dataset_grids", False):
            from rfdetr_pierre.datasets.save_grids import DatasetGridSaver
    ...
    def export(
        ...
        **kwargs,
        ...
        try:
            from rfdetr_pierre.deploy.export import export_onnx, make_infer_image, onnx_simplify
        except ImportError:
            logger.error(
