...
####---------------------------------------------------
# START OF INSERTED BY P.DENARIE FOR TT BACKED MLP LAYERS
####---------------------------------------------------
# Cache policy:
# - "none": no caching, reconstruct dense weights from TT factorization on every forward pass
# - "cpu": cache dense weights on CPU after first reconstruction, and move to device on
#   subsequent forward passes
# - "gpu": cache dense weights on GPU after first reconstruction, and reuse on subsequent forward passes (only recommended if the model is not shared across devices and the GPU memory overhead is acceptable)


class TTDinov2WithRegistersMLP(nn.Module):
    """TT-backed replacement for Dinov2WithRegistersMLP.

    The class stores a TT factorization of stacked dense weights:
    - stacked[0] = fc1.weight
    - stacked[1] = fc2.weight.T
    and reconstructs dense weights on-demand during forward.
    """
    ...
    def from_dense_mlp(
        ...
        try:
            ...
            raise ImportError(
                "tntorch_pierre is required to build TT-backed MLP layers."
            ) from exc
        ...
        with torch.no_grad():
            ...
            tt_tensor = tnp.Tensor(stacked, **tensor_kwargs)
    ...
    def _dense_weights(self, dtype: torch.dtype, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
        if self.cache_policy == "gpu":
            ...
            if not cache_hit:
                stacked = self.tt_tensor.torch().to(device=device, dtype=dtype)
        ...
        if self.cache_policy == "cpu":
            ...
            if not cache_hit:
                stacked_cpu = self.tt_tensor.torch().to(device="cpu")
            ...
            fc1_weight = self._cached_fc1_weight.to(device=device, dtype=dtype)
            fc2_weight = self._cached_fc2_weight.to(device=device, dtype=dtype)
            return fc1_weight, fc2_weight

        stacked = self.tt_tensor.torch().to(device=device, dtype=dtype)
        fc1_weight = stacked[0]
        fc2_weight = stacked[1].T
        return fc1_weight, fc2_weight
...
def build_tt_dinov2_mlp(
    ...
    return TTDinov2WithRegistersMLP.from_dense_mlp(
        mlp=mlp,
        eps=eps,
        ranks_tt=ranks_tt,
        ranks_cp=ranks_cp,
