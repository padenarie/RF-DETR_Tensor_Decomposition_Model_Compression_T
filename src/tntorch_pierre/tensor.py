...
import tntorch_pierre as tn


def _full_rank_tt(
    data: torch.Tensor, batch: Optional[bool] = False
):  #  NOTE: Naive TT formatting, don't even attempt to compress
    ...
    for n in range(1, N):
        if resh.shape[d1] < resh.shape[d2]:
            if batch:
                I = torch.eye(resh.shape[1], device=device, dtype=dtype).repeat(
                    resh.shape[0], 1, 1
                )
                result.append(
                    I.reshape(
                ...
                resh = torch.reshape(
                    ...
                    (
                        resh.shape[0],
                        resh.shape[1] * shape[n + 1],
                        resh.shape[2] // shape[n + 1],
                    ),
                )
            else:
                ...
                result.append(
                    I.reshape(
                        [resh.shape[0] // shape[n - 1], shape[n - 1], resh.shape[0]]
                    ).to(device)
                )
                resh = torch.reshape(
                    resh, (resh.shape[0] * shape[n], resh.shape[1] // shape[n])
                )
        else:
            if batch:
                result.append(
                ...
                I = torch.eye(resh.shape[2], device=device, dtype=dtype).repeat(
                    resh.shape[0], 1, 1
                )
                resh = I.reshape(
                    (
                ...
                result.append(
                    resh.reshape(
                        [resh.shape[0] // shape[n - 1], shape[n - 1], resh.shape[1]]
                    )
                )
                I = torch.eye(resh.shape[1], device=device, dtype=dtype)
                resh = I.reshape(
                    resh.shape[1] * shape[n], resh.shape[1] // shape[n]
                ).to(device)

    if batch:
        result.append(
            resh.reshape([resh.shape[0], resh.shape[1] // shape[N], shape[N], 1])
        )
    else:
...
class Tensor(object):
    ...
    def __init__(
        ...
        if isinstance(data, (list, tuple)):
            ...
            if not all(
                [min_dim <= d.dim() <= max_dim for d in data]
            ):  # add one dim for batch
                raise ValueError(
                    "All tensor cores must have 2 (for CP) or 3 (for TT) dimensions"
                )
            for n in range(len(data) - 1):
                if (
                    data[n + 1].dim() == max_dim
                    and data[n].shape[-1] != data[n + 1].shape[d1]
                ) or (
                    data[n + 1].dim() == min_dim
                    and data[n].shape[-1] != data[n + 1].shape[d2]
                ):
            ...
            if isinstance(data, np.ndarray):
                ...
                raise ValueError(
                    "A tntorch.Tensor may be built either from a list of cores, one NumPy ndarray, or one PyTorch tensor"
                )
            N = data.dim()
        if Us is None:
            Us = [None] * N
        ...
        if isinstance(data, torch.Tensor):
            ...
            if ranks_cp is not None:  # Compute CP from full tensor: CP-ALS
                ...
                if ranks_tucker is None:  # We initialize CP factor to HOSVD
                    if batch:
                        data_norms = torch.sqrt(
                            torch.sum(data**2, dim=list(range(1, data.dim())))
                        )
                        N = data.dim() - 1
                    else:
                    ...
                    for n in range(N):
                        ...
                        if batch:
                            ...
                            self.cores.append(
                                eigvecs[
                                    [[i] for i in range(len(idx))], :, idx
                                ].transpose(-1, -2)
                            )
                            if (
                                self.cores[-1].shape[2] < ranks_cp
                            ):  # Complete with random entries
                    ...
                    if batch:
                        ...
                        self.cores = [
                            ...
                            for sh in data.shape[1:]
                        ]
                    else:
                        data_norm = tn.norm(data)
                        self.cores = [
                            torch.randn(sh, ranks_cp, dtype=data.dtype, device=device)
                ...
                grams = [None] + [
                    self.cores[n].transpose(-1, -2) @ self.cores[n]
                ...
                if batch:
                    assert all(
                        [
                            len(self.cores[i]) == len(self.cores[i + 1])
                ...
                for iter in range(max_iter):
                    for n in range(self.dim()):
                        if batch:
                            khatri = torch.ones(
                                batch_size, 1, ranks_cp, dtype=data.dtype, device=device
                            )
                            prod = torch.ones(
                                batch_size,
                                ranks_cp,
                            ...
                            khatri = torch.ones(
                                1, ranks_cp, dtype=data.dtype, device=device
                            )
                            prod = torch.ones(
                                ranks_cp, ranks_cp, dtype=data.dtype, device=device
                            )
                            idxs = "ir,jr->ijr"
                            shape = [-1, ranks_cp]
                        ...
                        for m in range(self.dim() - 1, -1, -1):
                            if m != n:
                                prod *= grams[m]
                                khatri = torch.reshape(
                                    torch.einsum(idxs, (self.cores[m], khatri)), shape
                                )

                        unfolding = tn.unfolding(data, n, batch)

                        unf_khatri_t = (unfolding @ khatri).transpose(-1, -2)
                        self.cores[n] = torch.linalg.lstsq(
                            prod, unf_khatri_t
                    ...
                    if batch:
                        ...
                        errors.append(
                            (
                                torch.sqrt(
                                    torch.sum(err**2, dim=list(range(1, err.dim())))
                                )
                                / data_norms
                            ).mean()
                        )
                        ...
                        errors.append(
                            torch.norm(
                                data - tn.Tensor(self.cores, batch=self.batch).torch()
                            )
                            / data_norm
                        )
                    ...
                    if verbose:
                        print(
                            "iter: {: <{}} | eps: ".format(
                                iter, len("{}".format(max_iter))
                            ),
                            end="",
                        )
                        print("{:.8f}".format(errors[-1]), end="")
                        print(
                            " | total time: {:9.4f}".format(time.time() - start), end=""
                        )
                        if converged:
    ...
    def __add__(self, other: Union[Any, int, float]):
        ...
        if self.batch:
            ...
            ), f"Batch dim must match, got {self.shape[0]} and {other.shape[0]}"

        if self.dim() == 1:  # Special case
            return Tensor(
                [
                    self.decompress_tucker_factors().cores[0]
                    + other.decompress_tucker_factors().cores[0]
                ]
            )
    ...
    def __radd__(self, other: Union[Any, torch.Tensor]):

        if other is None:
            return self
        return self + other

    def __sub__(self, other: Union[Any, torch.Tensor]):

        return self + -1 * other

    def __rsub__(self, other: Union[Any, torch.Tensor]):
    ...
    def __mul__(self, other: Union[Any, int, float]):

        if not isinstance(other, Tensor):  # A scalar
            result = self.clone()
            factor = np.abs(other) ** (
                1 / self.dim()
            )  # We scale all cores by the same factor to prevent precision issues
            for n in range(self.dim()):
        ...
        for n in range(this.dim()):
            ...
            if (
                this.Us[n] is not None
                and other.Us[n] is not None
                and d1 < this.shape[n]
            ):
                cores.append(torch.reshape(torch.einsum(idx1, (core1, core2)), shape1))
                Us.append(
                    torch.reshape(torch.einsum(idx2, (this.Us[n], other.Us[n])), shape2)
                )
    ...
    def __truediv__(self, other: Union[Any, torch.Tensor]):

        return tn.cross(
            function=lambda x, y: x / y, tensors=[self, other], verbose=False
        )

    def __rtruediv__(self, other: Union[Any, torch.Tensor]):

        return tn.cross(
            function=lambda x, y: x / y,
            tensors=[tn.full_like(self, fill_value=other), self],
            verbose=False,
        )

    def __pow__(self, other: Union[Any, torch.Tensor]):

        return tn.cross(
            function=lambda x, y: x**y,
            tensors=[self, tn.full_like(self, fill_value=power)],
            verbose=False,
        )

    def __rmul__(self, other: Union[Any, torch.Tensor]):
    ...
    def __and__(self, other: Union[Any, torch.Tensor]):

        return self * other

    def __or__(self, other: Union[Any, torch.Tensor]):

        return self + other - self * other

    def __xor__(self, other: Union[Any, torch.Tensor]):

        return self + other - 2 * self * other

    def __eq__(self, other: Union[Any, torch.Tensor]):

        return tn.dist(self, other) <= 1e-14

    def __ne__(self, other: Union[Any, torch.Tensor]):
    ...
    @ranks_tucker.setter
    def ranks_tucker(self, value: int):
    ...
    def __repr__(self):
        ...
        if any(
            [
                (c.dim() == 3 and not self.batch) or (c.dim() == 4 and self.batch)
                for c in self.cores
            ]
        ):
            format.append("TT")
        if any(
            [
                (c.dim() == 2 and not self.batch) or (c.dim() == 3 and self.batch)
                for c in self.cores
            ]
        ):
            format.append("CP")
        ...
        if any([U is not None for U in self.Us]):
            ...
            for n in range(self.dim()):
                ...
                row[n * 4 - lenn // 2 + 2 : n * 4 - lenn // 2 + lenn + 2] = "{}".format(
                    shape[n]
                )
            s += "".join(row)
            s += "\n"
        ...
        for n in range(self.dim()):
            if self.Us[n] is None:
                ...
                row[n * 4 - lenr // 2 + 2 : n * 4 - lenr // 2 + lenr + 2] = "{}".format(
                    tuckerr[n]
                )
            else:
                row[n * 4 + 2 : n * 4 + 3] = "|"
        ...
        for n in range(self.dim()):
            ...
            lenn = len(nodestr)
            row[
                (n + 1) * 4 - (lenn - 1) // 2 : (n + 1) * 4 - (lenn - 1) // 2 + lenn
            ] = nodestr
        s += "".join(row[2:])
        s += "\n"
    ...
    def _process_key(self, key: Union[Sequence[int], torch.Tensor, int, Any]):
        ...
        for i in range(len(key)):
            if key[i] is Ellipsis:
                key = (
                    key[:i]
                    + [slice(None)] * (len(self.shape) - (len(key) - nonecount) + 1)
                    + key[i + 1 :]
                )
                break
        ...
        if len(self.shape) - (len(key) - nonecount) < 0:
            raise IndexError(
                "Too many index entries {} vs {}".format(
                    len(self.shape), len(key) - nonecount
                )
            )
    ...
    def __getitem__(self, key: Union[Sequence[int], torch.Tensor, int, Any]):
        """
        ...
        if isinstance(key, Tensor):
            if torch.abs(tn.sum(key) - 1) > 1e-8:
                raise ValueError(
                    "When indexing via a mask tensor, that mask should have exactly 1 accepting string"
                )
        ...
        if (
            ...
            or isinstance(key, torch.Tensor)
            and key.ndim == 2
        ):
        ...
        def insert_core(factors, core=None, key=None, U=None):
            ...
            if core is not None:
                if (
                    factors["int"] is not None
                ):  # There is a previous 1D/2D core (CP/Tucker) from an integer slicing
                    if U is None:
                        Us.append(None)
                        nCore = core[..., key, :]
                        if self.batch:
        ...
        def get_key(counter: int, key: Union[Sequence[int], torch.Tensor, int, Any]):
            if self.Us[counter] is None:
                ...
                if self.batch:
                    ...
                    if sl.dim() == 1:  # key is an int
                        ...
                        if self.cores[counter].dim() == 3:
                            return torch.einsum(
                                "ijk,aj->iak", (self.cores[counter], sl)
                            )
                        else:
            ...
            if this_mode == "none":
                if self.batch:
                    if batch_dim_processed:
                        ...
                        insert_core(
                            factors, core[:, :, None, :], key=slice(None), U=None
                        )
                    else:
                ...
                if self.batch:
                    ...
                    insert_core(
                        factors, self.cores[counter], key=key[i], U=self.Us[counter]
                    )
                ...
                if self.batch and first_index_dim == 0:
                    raise ValueError(
                        "Advanced indexing is prohibited for batch dimension"
                    )
        ...
        if last_mode == "index":
            ...
            if (
                len(cores) > 0
            ):  # We return a tensor: absorb existing cores with int factor
                if self.batch:
    ...
    def __setitem__(
        self, key: Union[Sequence[int], torch.Tensor, int, Any], value: Any
    ):
            ...
            if scalar:
                    ...
                    if k == len(value.shape) - 1:
                        ...
                        if sh == 1:
                            if value.shape[k] == sh:
                                ...
                                value = tn.Tensor(
                                    value.torch().reshape(new_shape), batch=self.batch
                                )
                ...
                if self.batch:
                    ...
                    if chunk.shape[1] != value.shape[i]:
                        raise ValueError(
                            "{}-th dimension mismatch in tensor assignment: {} (lhs) != {} (rhs)".format(
                                i, chunk.shape[1], value.shape[i]
                            )
                        )
                    if self.cores[i].dim() == 3:
                        add_core = torch.zeros(
        ...
        result = (
            self
            - tn.Tensor(subtract_cores, batch=self.batch)
            + tn.Tensor(add_cores, batch=self.batch)
        )
    ...
    def decompress_tucker_factors(
        self, dim: Optional[Union[str, Any]] = "all", _clone: Optional[bool] = True
    ):
        """
            ...
            if n in dim and self.Us[n] is not None:
                if self.batch:
                    if self.cores[n].dim() == 3:
                        cores.append(
                            torch.einsum("bjk,baj->bak", (self.cores[n], self.Us[n]))
                        )
                    else:
                        cores.append(
                            torch.einsum("bijk,baj->biak", (self.cores[n], self.Us[n]))
                        )
                else:
                    if self.cores[n].dim() == 2:
                        cores.append(
                            torch.einsum("jk,aj->ak", (self.cores[n], self.Us[n]))
                        )
                    else:
                        cores.append(
                            torch.einsum("ijk,aj->iak", (self.cores[n], self.Us[n]))
                        )
    ...
    def _cp_to_tt(self, factor: Optional[Union[None, Any]] = None):
        """
    ...
    def left_orthogonalize(self, mu: int):
        """
    ...
    def round_tucker(
        ...
        for mu in range(N - 1, -1, -1):
            if self.Us[mu] is None:
                ...
                if self.batch:
                    self.Us[mu] = (
                        torch.eye(self.shape[mu + 1], dtype=dtype)
            ...
            if self.batch:
                Q, R = torch.linalg.qr(
                    torch.reshape(
                        self.cores[mu].permute(0, 1, 3, 2),
                        [self.cores[mu].shape[0], -1, self.cores[mu].shape[2]],
                    )
                )
                self.cores[mu] = Q.reshape(
                    [
                ...
                Q, R = torch.linalg.qr(
                    torch.reshape(
                        self.cores[mu].permute(0, 2, 1), [-1, self.cores[mu].shape[1]]
                    )
                )
                self.cores[mu] = Q.reshape(
                    [self.cores[mu].shape[0], self.cores[mu].shape[2], -1]
                ).permute(0, 2, 1)
    ...
    def set_factors(
        ...
        for m in dim:
            if self.Us[m] is None:
                if self.batch:
                    self.Us[m] = tn.generate_basis(
                        name, (self.shape[m], self.shape[m])
                    ).repeat(self.shape[0], 1, 1)
                else:
                    self.Us[m] = tn.generate_basis(name, (self.shape[m], self.shape[m]))
            else:
                if self.batch:
                    tn.generate_basis(name, self.Us[m].shape).repeat(
                        self.shape[0], 1, 1
                    )
                else:
...
def _broadcast(a: Any, b: Any):

    if a.shape == b.shape:
        return a, b
    elif a.dim() != b.dim():
        raise ValueError(
            "Cannot broadcast: lhs has {} dimensions, rhs has {}".format(
                a.dim(), b.dim()
            )
        )
    result1 = a.repeat(
        *[int(round(max(sh2 / sh1, 1))) for sh1, sh2 in zip(a.shape, b.shape)]
    )
    result2 = b.repeat(
        *[int(round(max(sh1 / sh2, 1))) for sh1, sh2 in zip(a.shape, b.shape)]
    )
    return result1, result2


def _core_kron(a: Any, b: Any, batch: Optional[bool] = False):

    if batch:
        assert a.shape[0] == b.shape[0]
        c = a[:, :, None, :, :, None] * b[:, None, :, :, None, :]
        c = c.reshape(
            [a.shape[0], a.shape[1] * b.shape[1], -1, a.shape[-1] * b.shape[-1]]
        )
    else:
        c = a[:, None, :, :, None] * b[None, :, :, None, :]
